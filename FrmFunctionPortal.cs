...
namespace LiteToolSuite
{
    public partial class FrmFunctionPortal : Form
    {
        ...
        //private const int GRID_SIZE_ROW = 2;
        //private const int GRID_SIZE_COLUMN = 4;
        //private PictureBox[,] buttons = new PictureBox[GRID_SIZE, GRID_SIZE];
        //private readonly string[,] TEXTS = {
        //       { "上传Log", "Mqtt订阅", "查看Quota" },
        //       { "周期拍照", "车辆查看", "时间/时间戳" },
        //       { "登录Serverful站点", "", "退出程序" }
        //};
        ...
        private void combLang_SelectedIndexChanged(object sender, EventArgs e)
        {
            if (combLang.SelectedValue!=null && !string.IsNullOrEmpty(combLang.SelectedValue.ToString())) 
            {
                try
                {
                    ...
                    Program.TestDb.ExecuteScalar(sqliteSQL);  //保存语言选择至数据库
