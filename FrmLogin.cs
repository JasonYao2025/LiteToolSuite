...
namespace LiteToolSuite
{
    public partial class FrmLogin : Form
    {
        ...
        //public static  string DownloadFolder = @"C:\Log\";
        ...
        public FrmLogin()
        {
            ...
            //都以站点名为标准位置进行调整
            labelTitle.Location = new Point(label1.Left + 30, label1.Top - 100);  
        ...
        {
            var selectedLang = combLang.SelectedIndex;  //保存当前选择项,Controls.Clear() 会移除所有控件，而 InitializeComponent() 会重新创建控件，但不会自动恢复数据绑定或手动填充的数据。
            var selectedSite = combSite.SelectedIndex;
            ...
            Controls.Clear();
            InitializeComponent();
            //this.ResumeLayout(false); // 禁止立即计算布局
            //this.PerformLayout();     // 手动触发布局计算


            FilloutCombLang(selectedLang);
            FilloutCombSite(selectedSite);
           
        }

        private void combLang_SelectedIndexChanged(object sender, EventArgs e)
        {
            //FrmFunctionPortal.CultureCode = combLang.SelectedValue.ToString();

            //LanguageHelper.SetDefaultLang(FrmFunctionPortal.CultureCode);
            ...
            //}
            //else if (combLang.SelectedValue.ToString() == "zh-TW") 
            //{
            //    LanguageHelper.SetLang("zh-TW", this, typeof(FrmLogin));
            //}
            //else { }
            #endregion
        ...
        private void FilloutCombSite(int index)
        {
            ...
            try
            {
                ...
                var dataTable = Program.TestDb.ExecuteDataset(sqliteSQL,null).Tables[0];
        ...
        private void btRemoveSite_Click(object sender, EventArgs e)
        {
            ...
            if (!string.IsNullOrEmpty(siteURL)) 
            {
                ...
                var dt = Program.TestDb.ExecuteDataTable(sqliteSQL);
                if (dt.Rows.Count <= 1)
                {                   
                ...
                if (result == DialogResult.Yes)
                {
                    try
                    {
                        ...
                        var val = Program.TestDb.ExecuteNonQuery(sqliteSQL);
