...
namespace LiteToolSuite
{
    public partial class FrmVehicle : Form
    {
            ...
            if (VehicleOperation.GetVehicleList(FrmFunctionPortal.URL, FrmFunctionPortal.Token, out vehicles))
            {
                if (vehicles.Count > 0)
                {
                    ...
                    foreach (var vehicle in vehicles)
                    {
                        ...
                        if (!vehicle.Value.Contains("没有绑定AI主机") && !string.IsNullOrEmpty(vehicle.Value))
                        {
        ...
        private void btnVehicleQRCode_Click(object sender, EventArgs e)
        {
            ...
            {
                MessageBox.Show("主机没有型号的车辆不能生成车辆二维码", "提示", MessageBoxButtons.OK, MessageBoxIcon.Information);
                return;
            }
